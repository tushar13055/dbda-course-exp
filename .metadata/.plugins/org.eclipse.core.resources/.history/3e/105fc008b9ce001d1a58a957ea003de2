...
public class Test6 {
	...
	public static void main(String[] args) {
		...
		// p.study();//javac err : javac resolves the method binding by the type of ref
		// :Person
		...
		if(p instanceof Faculty) //false
		((Faculty) p).teach();// no javac err , run time err : ClassCastException : Student can't be cast to
								// Faculty
		else 
